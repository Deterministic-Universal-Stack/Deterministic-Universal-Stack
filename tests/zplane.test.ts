...
} from "../packages/zplane/src/index.js";
import { typeScriptAdapter } from "../packages/zplane/src/adapters/typescript.js";
import {
  createZPlaneCoordinator,
} from "../packages/zplane/src/runtime/index.js";
...
describe("Z Plane - Polyglot Interoperability", () => {
  ...
  describe("TypeScript Adapter", () => {
    ...
    it("should verify determinism", async () => {
      const report = await typeScriptAdapter.verifyDeterminism(
        "test-hash",
        10
      );
  ...
  describe("Z Plane Runtime Coordinator", () => {
    ...
    it("should list registered adapters", async () => {
      ...
      expect(adapters.some((a) => a.language === RuntimeLanguage.TYPESCRIPT)).toBe(
        true
      );
    });
    ...
    it("should serialize and deserialize data", async () => {
      ...
      const serialized = await coordinator.serialize(
        original,
        SerializationFormat.JSON
      );
      const deserialized = await coordinator.deserialize(
        serialized,
        SerializationFormat.JSON
      );
