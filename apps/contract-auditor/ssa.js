// ============================================================
// GEMFLOW Ω  ·  js/core/ssa.js
// Static Single Assignment builder: AST → SSA form
// Each variable assignment gets a unique versioned name.
// Phi-nodes inserted at control flow join points.
// ============================================================
...
export class SSAInstr {
  constructor(op, dest, args, meta = {}) {
    this.op   = op;
    this.dest = dest;   // SSAVar or null
    this.args = args;   // (SSAVar | string | number)[]
    this.meta = meta;   // { line, flags }
  }
  toString() {
...
export class PhiNode {
  constructor(dest, incoming) {
    this.dest     = dest;       // SSAVar
    this.incoming = incoming;   // Map<blockId, SSAVar>
  }
  toString() { return `${this.dest} = φ(${[...this.incoming.values()].join(', ')})`; }
}
...
export class SSAFunction {
  constructor(name, contract) {
    ...
    this.entry    = null;
    this.exit     = null;
    this._ver     = {};      // original name → version counter
    this._env     = {};      // original name → current SSAVar
    this.params   = [];
    this.returns  = [];
    this.flags    = {};
  }
...
export class SSAModule {
  constructor(contractName) {
    ...
    this.globals      = new Map(); // stateVarName → { type, slot }
    this.stats        = {};
  }
}
...
function liftBodyTokens(fn, blk, tokens) {
  ...
  while (i < tokens.length) {
    ...
    if (t.value === 'require' || t.value === 'assert') {
      const op = t.value.toUpperCase(); eat();
...
function buildFuncSSA(func, globals) {
  ...
  fn.addBlock(body);
  entry.succs.push(body.id); body.preds.push(entry.id);

  if (func.block?.body) liftBodyTokens(fn, body, func.block.body);

  // Insert phi-node placeholder at body entry if there's a back-edge (loops)
  // (simplified: just mark phi placeholder for state vars that are both read+written)
  for (const [varName] of globals) {
    const writes = body.instrs.filter(i => i.meta?.stateVar === varName && i.op === 'STORE');
    const reads  = body.instrs.filter(i => i.meta?.stateVar === varName && i.op === 'LOAD');
    if (writes.length && reads.length) {
      body.phis.push(new PhiNode(fn.def(varName), new Map([
        [entry.id, fn.use(varName)],
        [body.id,  fn.use(varName)]
      ])));
    }
  }

  // CEI violation: EXTCALL before STORE in same block
  ...
  fn.addBlock(exit);
  body.succs.push(exit.id); exit.preds.push(body.id);
...
export function buildSSA(ast) {
  ...
  for (const contract of ast.contracts) {
    ...
    // Stats
    let totalInstrs = 0, totalBlocks = 0;
    for (const [, f] of mod.functions) {
      totalBlocks += f.blocks.length;
      for (const b of f.blocks) totalInstrs += b.instrs.length;
    }
    mod.stats = { totalInstrs, totalBlocks, globals: mod.globals.size };
    modules.push(mod);
  }
