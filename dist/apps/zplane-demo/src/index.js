 ...
 */
import { RuntimeLanguage, SerializationFormat, } from "@dus/zplane";
...
async function initializeAdapters() {
    ...
    // Register operations
    await typeScriptAdapter.registerOperation("validate-input", async (input, _format, context) => {
    ...
    await typeScriptAdapter.registerOperation("format-output", async (input, _format, context) => {
...
async function executeWorkflow(coordinator) {
    ...
    if (result.stepResults.size > 0) {
        ...
        for (const [stepId, stepResult] of result.stepResults) {
            ...
            console.log(`    Execution Time: ${stepResult.metrics.executionTimeMs}ms`);
...
// Verify determinism
async function verifyDeterminism(coordinator) {
    ...
    if (report.discrepancies.length > 0) {
        ...
        for (const disc of report.discrepancies) {
            console.log(`  ${disc.language1} vs ${disc.language2} at event ${disc.divergencePoint}`);
        }
    }
}
// Monitor adapter health
async function monitorHealth(coordinator) {
    ...
    for (const adapter of adapters) {
        ...
        console.log(`  Avg Execution Time: ${health.metrics.averageExecutionTimeMs.toFixed(2)}ms`);
    }
}
// Main
async function main() {
    try {
        ...
        console.log("This demo shows how different languages participate in DUS determinism.\n");
