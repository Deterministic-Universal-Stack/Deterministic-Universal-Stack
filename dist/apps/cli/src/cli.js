...
`,
    ...
    await writeFile(join(projectDir, 'src', 'index.ts'), templates[template] || templates.basic);
    // Create README
    const readme = `# ${name}
...
// EVENT INSPECTION
// ============================================================================
async function inspectEvents(logPath, options) {
    ...
    // Timeline analysis
    const timestamps = events.map((e) => e.metadata.timestamp).sort((a, b) => a - b);
    ...
    // Causal structure
    const roots = events.filter((e) => e.parents.length === 0);
    const leaves = new Set(events.map((e) => e.id));
    for (const event of events) {
    ...
    console.log(`   Avg parents per event: ${(events.reduce((sum, e) => sum + e.parents.length, 0) / events.length).toFixed(2)}`);
    // Node distribution
...
// PERFORMANCE PROFILING
// ============================================================================
async function profileEvents(logPath) {
...
// ============================================================================
async function visualizeEvents(logPath, options) {
    ...
    console.log(`   Edges: ${events.reduce((sum, e) => sum + e.parents.length, 0)}`);
}
// ============================================================================
// TESTING FRAMEWORK
// ============================================================================
