...
import { DUS, canonicalHash, stringifyWithBigInt, topologicalSort, createState, } from "@dus/core";
import { replayEvents } from "@dus/replay";
import { DeterministicAgentRuntime, DeterministicProgramRuntime, } from "@dus/runtime";
import { FileSystemEventStore } from "@dus/storage";
import path from "node:path";
import { readFile, writeFile } from "node:fs/promises";
import { fileURLToPath } from "node:url";
async function handleEventEmit(nodeId, reducerVersion, eventInput, options) {
...
async function handleExecute(planPath, options) {
    ...
    const reducer = eval(plan.reducer);
    // Load events
    const events = typeof plan.events === "string"
        ? JSON.parse(await readFile(plan.events, "utf8"))
        : plan.events;
    const runtime = new DUS(plan.nodeId, reducer, {
    ...
    if (options.checkpoint) {
        const trace = replayEvents(createState((plan.initialValue ?? {})), events, reducer, options.checkpoint);
...
// REPLAY SURFACE
// ==============================================================================
async function handleReplay(logPath, options) {
    ...
    const initialState = options.checkpoint
        ? JSON.parse(await readFile(options.checkpoint, "utf8"))
        : createState({});
...
async function handleProject(configPath) {
    ...
    const projectFn = eval(config.projection);
...
// SCHEDULER INTERFACE
// ==============================================================================
async function handleSchedule(runtimeType, options) {
    console.log(`🗓 Initializing ${runtimeType} runtime...`);
    if (runtimeType === "agent") {
        const runtime = new DeterministicAgentRuntime(options.nodeId, options.runId, options.branch ?? "main");
        if (options.steps) {
...
// GRAPH MANIPULATOR
// ==============================================================================
async function handleGraph(logPath, operation, options) {
    const logRaw = await readFile(logPath, "utf8");
    const log = JSON.parse(logRaw);
    const events = log.events;
    ...
    if (operation === "visualize") {
        ...
        const filterFn = eval(options.filter);
...
async function handleInspect(logPath, queryPath, options) {
    const logRaw = await readFile(logPath, "utf8");
    const log = JSON.parse(logRaw);
    const events = log.events;
    ...
    if (query.type === "causal-path") {
        ...
        const { invariant } = query.params;
        const checkFn = eval(invariant);
...
program
    ...
    .action(async (opts) => {
    await handleEventEmit(opts.nodeId, opts.reducerVersion, {
        type: opts.type,
